...
public:
    CustomCellFactory()
        : AbstractCardiacCellFactory<2>(), 
          mpStimulus(new SimpleStimulus(-900e4, 2))
    {
    }
...
public: // Tests should be public!
    ...
    void TestWithBathAndElectrodes()
    {
        ...
        double h=0.0005;
        mesh.ConstructRegularSlabMesh(h, 0.045 /*length*/, 0.016 /*width*/);
        HeartConfig::Instance()->SetOutputUsingOriginalNodeOrdering(true);
        HeartConfig::Instance()->SetSimulationDuration(100.0);  //ms
        HeartConfig::Instance()->SetOutputDirectory("ScaledIN4BCB100");
        HeartConfig::Instance()->SetOutputFilenamePrefix("ScaledIN4BCB100");
             ...
             ++iter)
        {
			...
			else if (sqrt((x-0.02625)*(x-0.02625) + (y-0.008)*(y-0.008)) < 0.004) 
            {
                if (sqrt((x-0.02625)*(x-0.02625) + (y-0.008)*(y-0.008)) < 0.002)
				{	
				iter->SetAttribute(bath_id1);
				}
				else
			...
			else if (sqrt((x-0.03375)*(x-0.03375) + (y-0.008)*(y-0.008)) < 0.004) 
            {
                if (sqrt((x-0.03375)*(x-0.03375) + (y-0.008)*(y-0.008)) < 0.002)
				{	
				iter->SetAttribute(bath_id1);
				}
				else
