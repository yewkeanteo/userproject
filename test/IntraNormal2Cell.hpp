...
public:
    CustomCellFactory()
        : AbstractCardiacCellFactory<2>(), 
          mpStimulus(new SimpleStimulus(-9e5, 2))
    {
    }
    ...
    AbstractCvodeCell* CreateCardiacCellForTissueNode(Node<2>* pNode)
    {
		...
		*/
         if ((x<0.31+1e-6) && (y<0.21+1e-6))
			{
			if ((x>0.29+1e-6) && (y>0.19+1e-6))
				{
...
public: // Tests should be public!
    ...
    void TestWithBathAndElectrodes()
    {
        ...
        HeartConfig::Instance()->SetOutputDirectory("IntraNormal2Cell500");
        HeartConfig::Instance()->SetOutputFilenamePrefix("IntraNormal2Cell500");
