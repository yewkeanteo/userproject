...
public: // Tests should be public!
    ...
    void TestWithBathAndElectrodes()
    {
        ...
        HeartConfig::Instance()->SetOutputDirectory("NormalConductance1100h1");
        HeartConfig::Instance()->SetOutputFilenamePrefix("NormalConductance1100h1");
             ...
             ++iter)
        {
            ...
            if (sqrt((x-0.355)*(x-0.355) + (y-0.2)*(y-0.2)) < 0.1)
            {
                //IDs default to 0, but we want to be safe
                iter->SetAttribute(tissue_id);
            }
			else if (sqrt((x-0.445)*(x-0.445) + (y-0.2)*(y-0.2)) < 0.1)
			{
                //IDs default to 0, but we want to be safe
                iter->SetAttribute(tissue_id);				
			}
